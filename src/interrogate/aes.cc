 ...
 * 
 * 
 * Modified by Leon Abascal as an adaptation to the KeyReaper project
 * ==========================================================================
 */
...
vector<vector<BYTE>> aes_search(interrogate_context *ctx, unsigned char *buffer)  {
  auto found_keys = vector<vector<BYTE>>();
	unsigned int i;
	...
	for (i = ctx->from; i < ctx->filelen - kssize; i++) {
		...
		if (memcmp(ks, &buffer[i], kssize) == 0) {
			ctx->count++;
      auto key = vector<BYTE>(ctx->keysize/8, 0);
      memcpy(key.data(), ks, key.size());
